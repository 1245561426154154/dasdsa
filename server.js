...
app.get("/script/:id",(r,s)=>{const c=scripts[r.params.id];if(!c)return s.send("print'fake'");if(!r.headers["user-agent"]?.includes("Roblox"))return s.send("print'fake'");const k=crypto.randomBytes(32),e=encrypt(c,k);let ek="";const kh=k.toString("hex");for(let i=0;i<kh.length;i++)ek+=String.fromCharCode(kh.charCodeAt(i)^SHARED_SECRET.charCodeAt(i%SHARED_SECRET.length));s.json({data:e.data,iv:e.iv,key:Buffer.from(ek).toString("hex")})})
